...
class InteractiveGelAnalyzer:
    ...
    def save_figure(self, fig, analysis):
        ...
        try:
            ...
            if not self.filepath:
                print("Warning: No filepath available, skipping save")
                return
            ...
            # Get coordinates for filename with ratio
            x1, y1, x2, y2 = analysis['coords']
            ratio = analysis.get('fragment_ratio', 0)
            filename_base = f"{ratio:.4f}_{int(x1)}_{int(x2)}"
    ...
    def plot_results(self, analysis):
        ...
        if analysis.get('plasmid_coords'):
            ...
            print(f"  Drawing plasmid rectangle: ({px1}, {py1}) to ({px2}, {py2})")
        
        # Fragment (BLUE) - 显示原始框选区域
        if analysis.get('fragment_coords'):
            ...
            print(f"  Drawing fragment rectangle: ({fx1}, {fy1}) to ({fx2}, {fy2})")
        ...
        # Build title with fraction as third line
        ...
        if analysis['fragment_ratio'] > 0:
            title_text += f'\nFraction: {analysis["fragment_ratio"]:.2f}'
...
class GelAnalysisGUI:
    ...
    def __init__(self, root, image_data, filename, filepath=None):
        ...
        self.panel_number = tk.IntVar(value=1)  # Number of panels to divide into
        self.rectangles = {
    ...
    def create_widgets(self):
        ...
        instructions = [
            ...
            "4. Use arrow keys to fine-tune selection (Shift+Arrow for 10px steps)",
            "5. Click 'Analyze Panel' to process and view results (n panels will be auto-divided)"
        ]
        ...
        # === PANEL NUMBER INPUT ===
        panel_frame = ttk.LabelFrame(control_frame, text="Panel Configuration", padding="10")
        panel_frame.grid(row=6, column=0, columnspan=6, sticky=(tk.W, tk.E), pady=10)
        
        ttk.Label(panel_frame, text="Number of Panels (horizontal division):",
                 style='Large.TLabel').grid(row=0, column=0, padx=10, sticky=tk.W)
        panel_spinbox = ttk.Spinbox(panel_frame, from_=1, to=20, textvariable=self.panel_number,
                                    width=10, font=('Arial', 13))
        panel_spinbox.grid(row=0, column=1, padx=10, sticky=tk.W)
        
        # === SELECTION MODE ===
        mode_frame = ttk.LabelFrame(control_frame, text="Selection Mode", padding="10")
        mode_frame.grid(row=7, column=0, columnspan=6, sticky=(tk.W, tk.E), pady=10)
        ...
        self.status_label.grid(row=9, column=0, columnspan=6, pady=10)
        ...
        # Bind keyboard events for fine-tuning
        self.root.bind('<Left>', lambda e: self.adjust_selection(-1, 0))
        self.root.bind('<Right>', lambda e: self.adjust_selection(1, 0))
        self.root.bind('<Up>', lambda e: self.adjust_selection(0, -1))
        self.root.bind('<Down>', lambda e: self.adjust_selection(0, 1))
        self.root.bind('<Shift-Left>', lambda e: self.adjust_selection(-10, 0))
        self.root.bind('<Shift-Right>', lambda e: self.adjust_selection(10, 0))
        self.root.bind('<Shift-Up>', lambda e: self.adjust_selection(0, -10))
        self.root.bind('<Shift-Down>', lambda e: self.adjust_selection(0, 10))
    ...
    def redraw_rectangles(self):
        ...
        # Draw panel with subdivisions if n > 1
        if self.rectangles['panel']['coords']:
            x1, y1, x2, y2 = self.rectangles['panel']['coords']
            n_panels = self.panel_number.get()
            
            # Draw outer rectangle
            rect = Rectangle((x1, y1), x2-x1, y2-y1,
                           fill=False, edgecolor='red',
                           linewidth=4, label='Panel')
            self.ax.add_patch(rect)
            
            # Draw subdivision lines if n > 1
            if n_panels > 1:
                panel_width = (x2 - x1) / n_panels
                for i in range(1, n_panels):
                    sub_x = x1 + i * panel_width
                    self.ax.plot([sub_x, sub_x], [y1, y2], 'r--', linewidth=2, alpha=0.7)
        
        # Draw plasmid and fragment rectangles
        for mode in ['plasmid', 'fragment']:
            data = self.rectangles[mode]
            if data['coords']:
    ...
    def on_mouse_press(self, event):
        ...
        # Remove existing rectangle for this mode to prepare for new one
    ...
    def on_mouse_move(self, event):
        """Handle mouse move event - draw temporary rectangle while dragging"""
        ...
        # Draw new temporary rectangle (simple, no subdivisions)
        ...
        rect = Rectangle((x1, y1), x2-x1, y2-y1,
                        fill=False, edgecolor=color, linewidth=3, linestyle='-', alpha=0.8)
        self.rectangles[mode]['rect_id'] = self.ax.add_patch(rect)
        
        # Use blit for fast update if possible, otherwise use draw_idle
        try:
    ...
    def on_mouse_release(self, event):
        ...
        # Update display to show panel subdivisions if applicable
        self.update_image_display()
        self.canvas.draw()
    ...
    def adjust_selection(self, dx, dy):
        """Adjust current selection by dx, dy pixels using keyboard arrows"""
        mode = self.selection_mode.get()
        if not self.rectangles[mode]['coords']:
            self.status_label.config(text=f"No {mode} selection to adjust", foreground="orange")
            return
        
        x1, y1, x2, y2 = self.rectangles[mode]['coords']
        
        # Adjust both corners by the same amount (moves the rectangle)
        x1 += dx
        x2 += dx
        y1 += dy
        y2 += dy
        ...
        h, w = self.analyzer.image.shape
        x1 = max(0, min(x1, w))
        x2 = max(0, min(x2, w))
        y1 = max(0, min(y1, h))
        y2 = max(0, min(y2, h))
        
        self.rectangles[mode]['coords'] = (x1, y1, x2, y2)
        self.update_image_display()
        self.canvas.draw()
        ...
        if dy > 0: direction += (" " if direction else "") + "down"
        elif dy < 0: direction += (" " if direction else "") + "up"
        
        self.status_label.config(
            text=f"{mode.capitalize()} moved {direction} ({abs(dx) if dx else abs(dy)} px)",
            foreground="blue"
        )
    
    def get_overlap_coords(self, coords, panel_coords):
        """Calculate the overlap between coords and panel_coords"""
        if not coords:
            return None
    ...
    def analyze_selection(self):
        """Analyze the selected panel, divided into n horizontal panels"""
        try:
            ...
            # Get number of panels to divide into
            n_panels = self.panel_number.get()
            
            self.status_label.config(text=f"Analyzing {n_panels} panel(s)...", foreground="blue")
            self.root.update()
            
            # Calculate panel width for horizontal division
            panel_width = (x2 - x1) / n_panels
            
            # Analyze each panel independently
            for panel_idx in range(n_panels):
                # Calculate coordinates for this sub-panel
                sub_x1 = int(x1 + panel_idx * panel_width)
                sub_x2 = int(x1 + (panel_idx + 1) * panel_width)
                sub_panel_coords = (sub_x1, y1, sub_x2, y2)
                
                # Adjust plasmid and fragment coordinates to overlap with this sub-panel
                sub_plasmid_coords = self.get_overlap_coords(plasmid_coords, sub_panel_coords) if plasmid_coords else None
                sub_fragment_coords = self.get_overlap_coords(fragment_coords, sub_panel_coords) if fragment_coords else None
                
                # Perform analysis for this panel
                analysis = self.analyzer.analyze_panel(sub_panel_coords, sub_plasmid_coords, sub_fragment_coords)
                
                # Show results in a new window
                self.show_analysis_results(analysis, panel_number=panel_idx + 1, total_panels=n_panels)
            
            # Update status
            self.status_label.config(text=f"Analysis complete! {n_panels} panel(s) processed.", foreground="green")
    ...
    def show_analysis_results(self, analysis, panel_number=None, total_panels=None):
        """Display analysis results in new window with 5 subplots"""
        title = "Analysis Results - 5 Subplots"
        if panel_number is not None and total_panels is not None:
            title += f" (Panel {panel_number}/{total_panels})"
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if not os.path.exists(selected_file):
            print(f"\n❌ ERROR: File not found: {selected_file}")
            print(f"   当前工作目录: {os.getcwd()}")
            sys.exit(1)
    else:
        # Find SCN files in current directory
        scn_files = sorted(glob.glob("*.scn"))
        
        if not scn_files:
            print("\nERROR: No SCN files found!")
            print("Please run in directory containing .scn files")
            print("Or provide file path as argument: python script.py <file.scn>")
            sys.exit(1)
